...
const MyApplicationsPage = () => {
  ...
  // Fetch user details and leave applications when user data is available
  useEffect(() => {
    if (!loading && user) {
      fetchUserDetails(user.id, user.role);
      fetchLeaveApplications(user.email);
      fetchLeaveBalance(user.email);
  ...
  const fetchLeaveBalance = async (email) => {
    try {
      ...
      if (data && data.leaveBalance) {
        setLeaveBalance(data.leaveBalance); // Update leave balance with the correct values
      }
    } catch (error) {
  ...
  const fetchLeaveApplications = async (email) => {
    try {
      const response = await fetch(`http://localhost:5000/api/leaves/${email}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
      ...
      if (data && Array.isArray(data.leaves)) {
        setLeaveApplications(data.leaves); // Only display the user's leave applications
      }
    } catch (error) {
  ...
  if (!user) {
    return null; // Optionally return a message or redirect if user is not available
  }
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="bg-gradient-to-r from-teal-500 to-teal-600 text-white p-6 rounded shadow-md flex flex-col items-center justify-between">
            ...
            <div className="flex justify-between w-full mt-4">
              ...
              <p>{user.email}</p> {/* Assuming role is available */}
            </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tutor's Remarks</th>
