...
const ApplyLeave1 = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      data.append('email', user?.email || ''); // Add email to FormData
      ...
      try {
        ...
        if (!response.ok) {
          const text = await response.json(); // Parse the error response as JSON
          throw new Error(text.error || 'Something went wrong');
        }

        alert('Teacher leave application submitted successfully');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-blue-900">
      <div className="bg-white p-6 rounded shadow-md w-full max-w-md">
        <h2 className="text-3xl font-bold mb-4 text-center text-blue-900">Apply Teacher Leave</h2>
        {errorMessage && <div className="text-red-500 text-sm mb-4">{errorMessage}</div>} {/* Display error message */}
